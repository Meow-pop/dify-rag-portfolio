...
一个面向 AI 应用开发岗位的可复现作品集项目：在本地部署 Dify，接入 DeepSeek 与 Ollama，并逐步实现带引用、可评测、可通过 API 调用的通用企业知识库助手。

项目重点是本地 AI 基础设施、模型接入、RAG 检索、质量评测、安全边界和应用集成，不属于电商项目。知识库中的行业内容只是可替换的合成测试资料，不限定系统用途。
...
当前知识库使用四份虚构企业资料验证文档切分、向量检索和问答召回。它们只是一组不含真实业务数据的测试语料，可以替换为公司制度、产品说明、技术文档、项目资料或其他获得授权的内容。项目评价的是部署与 RAG 能力，而不是某个行业的业务系统。
